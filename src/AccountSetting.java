	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(c_password == "" && password == "")
		{
			try
			{
				...
				{
					session.setAttribute("result", new Display(Display.Type.SUCCESS).getHtml("Account Settings updated!"));
				}
				else
				{
					session.setAttribute("result", new Display(Display.Type.ERROR).getHtml("Failed to update your account!"));
				}
			}
			catch(Exception ex)
			{
		...
		else
		{
			...
			{
				session.setAttribute("result", new Display(Display.Type.WARNING).getHtml("Both password fields are required to change password!"));
			}
			...
			{
				session.setAttribute("result", new Display(Display.Type.WARNING).getHtml("Passwords don't match, Please try again!"));
			}
		}
