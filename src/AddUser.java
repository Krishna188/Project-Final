...
public class AddUser extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(role.equals("ADMIN") || role.equals("STUDENT") || role.equals("TEACHER"))
		{
			...
			if(username == null || password == null ||fname == null || lname == null || password == " " || fname == " " || lname == " " || username == " " )
			{
				session.setAttribute("add_result", new Display(Display.Type.ERROR).getHtml("All the fields are required to add user."));
			}
			else
			{
				try
				{
					...
					{
						session.setAttribute("add_result", new Display(Display.Type.SUCCESS).getHtml("User has been added!"));
					}
					else
					{
						session.setAttribute("add_result", new Display(Display.Type.ERROR).getHtml("Failed to add user!"));
					}
				}
				catch(Exception ex)
				{
		...
		{
			session.setAttribute("add_result", new Display(Display.Type.ERROR).getHtml("Invalid request please try again!"));
		}
